...
class DatasetValidator:
    ...
    def _validate_category_consistency(self) -> bool:
        ...
        logger.info("  Validating category consistency...")
    ...
    def _validate_category_ids(self) -> bool:
        ...
        if min_id == 1:
            logger.warning("  Category IDs start from 1 (dataset format)")
        ...
        if max_id > 10000:
            ...
            logger.warning(f"  Very high category ID: {max_id}")
    ...
    def _validate_detectron2_compatibility(self) -> bool:
        ...
        if 'category_ids' in self.validation_results:
            ...
            if not cat_results['detectron2_compatible']:
                if cat_results['needs_remapping']:
                    ...
                    logger.warning("  Category IDs need remapping: 1-based -> 0-based")
                else:
        ...
        if valid and not issues:
            ...
            logger.warning(f"  Detectron2 compatibility issues (can be fixed): {len(issues)}")
                ...
                for i, ann in enumerate(data['annotations'][:5]):
                    ...
                    if 'bbox' in ann:
                        ...
                        if not (isinstance(bbox, list) and len(bbox) == 4):
                            ...
                            logger.warning(f"  Negative bbox values in {split}: {bbox}")
