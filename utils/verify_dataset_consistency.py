...
def check_category_id_ranges(data):
    ...
    for split, split_data in data.items():
        ...
        if gaps:
            print(f"   ⚠️  Missing IDs: {len(gaps)} gaps in range")
            if len(gaps) <= 20:
        ...
        if 0 in cat_ids:
            print(f"   ⚠️  Category ID 0 present (often indicates background/unknown)")
        
        if min(cat_ids) > 1:
            print(f"   ⚠️  Category IDs don't start at 0 or 1 (starts at {min(cat_ids)})")
...
def compare_with_predictions():
    ...
    print(f"   ⚠️  Missing category predictions: {len(missing_predictions)}")
    
    if wrong_predictions:
        print(f"   Non-existent category IDs being predicted: {sorted(wrong_predictions)}")
        if 0 in wrong_predictions:
            print("      ⚠️  Category 0 predictions suggest background/unknown confusion")
