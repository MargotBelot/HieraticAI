...
class DatasetValidator:
    ...
    def validate_all(self) -> bool:
        ...
        for step_name, step_func in steps:
            ...
            try:
                result = step_func()
                if not result:
                    logger.warning(f"  [WARNING]  {step_name} validation completed with warnings")
                else:
    ...
    def _validate_category_consistency(self) -> bool:
        ...
        for split, categories in categories_by_split.items():
            ...
            if categories != reference_categories:
                inconsistencies.append(f"{split} has different categories than {reference_split}")
    ...
    def _validate_category_ids(self) -> bool:
        ...
        if min_id == 1:
            logger.warning("  [WARNING]  Categories use 1-based indexing (starting at 1)")
            logger.warning("  [WARNING]  Detectron2 expects 0-based indexing (starting at 0)")
            logger.warning("  [WARNING]  Category ID remapping is required during training")
            needs_remapping = True
            detectron2_compatible = False
        elif min_id == 0:
            logger.info("  [PASS] Categories use 0-based indexing (Detectron2 compatible)")
            needs_remapping = False
            ...
            try:
                ...
                for ann in annotations:
                    ...
                    for field in required_fields:
                        if field not in ann:
                            issues.append(f"{split}: Annotation {ann.get('id', 'unknown')} missing '{field}'")

                    # Check category_id validity
                    if ann.get('category_id') not in category_ids:
                        issues.append(f"{split}: Annotation {ann['id']} has invalid category_id {ann['category_id']}")
            ...
            try:
                ...
                for img_info in images:
                    ...
                    if not img_path.exists():
                        issues.append(f"{split}: Image file missing: {img_info['file_name']}")
                    else:
                        total_image_files += 1
    ...
    def _validate_data_distribution(self) -> bool:
        ...
        for split, stats in distribution.items():
            logger.info(f"  {split:5s}: {stats['images']:4d} images, {stats['annotations']:4d} annotations "
    ...
    def _validate_detectron2_compatibility(self) -> bool:
        ...
        issues = []
        needs_category_remapping = self.validation_results.get('category_ids', {}).get('needs_remapping', False)

        if needs_category_remapping:
            issues.append("Category IDs need remapping from 1-based to 0-based for Detectron2")
            logger.warning("  [WARNING]  Use category ID remapping during training (CategoryRemappingDatasetMapper)")
    ...
    def _generate_summary(self) -> Dict:
        ...
        if self.validation_results['detectron2_compatibility'].get('needs_category_remapping'):
            summary['recommendations'].append(
    ...
    def get_category_mapping_info(self) -> Dict:
        ...
        """
        return {
            'needs_remapping': self.validation_results.get('category_ids', {}).get('needs_remapping', False),
            'offset': -1 if self.validation_results.get('category_ids', {}).get('needs_remapping', False) else 0,
            'num_categories': self.validation_results.get('category_ids', {}).get('num_categories', 0)
        }
