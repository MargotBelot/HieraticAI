...
python3 -c "from utils.dataset_validator import DatasetValidator; v = DatasetValidator('hieroglyphs_dataset'); result = v._validate_file_structure(); print('  [PASS] PASS' if result else '  [WARNING] PASS (with warnings)')" 2>/dev/null || echo "  [FAIL] FAIL"

# Test 4: Category remapping detection
echo "Test 4: Category remapping detection..."
python3 -c "from utils.dataset_validator import DatasetValidator; v = DatasetValidator('hieroglyphs_dataset'); v._validate_file_structure(); v._validate_category_consistency(); v._validate_category_ids(); info = v.get_category_mapping_info(); print('  [PASS] PASS (needs_remapping=' + str(info['needs_remapping']) + ', offset=' + str(info['offset']) + ')')" 2>/dev/null || echo "  [FAIL] FAIL"
...
echo "See docs/maintenance/FIXES_APPLIED.md for details."
