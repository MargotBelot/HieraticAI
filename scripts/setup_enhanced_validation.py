...
def check_aku_data():
    ...
    aku_path = Path("/Users/margot/Desktop/ALP_project/AKU Westcar Scraping")
    
    if not aku_path.exists():
        print(f"⚠️  AKU data not found at: {aku_path}")
    ...
    if not json_path.exists():
        print(f"⚠️  JSON folder not found: {json_path}")
        return False
...
def main():
    ...
    if not indexer_success:
        print("\n⚠️  AKU indexer failed, but continuing with setup...")
    ...
    if index_path.exists():
        try:
            ...
            print(f"⚠️  Could not read AKU index: {e}")
