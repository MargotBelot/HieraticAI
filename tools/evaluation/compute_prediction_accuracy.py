...
class PredictionAccuracyAnalyzer:
    ...
    def __init__(
        self, dataset_path: str, predictions_file: str, iou_threshold: float = 0.5
    ):
        """
    ...
    def compute_accuracy_metrics(self) -> Dict:
        ...
        for image_id in gt_by_image.keys():
            ...
            for pred in predictions:
                ...
                if best_iou >= self.iou_threshold and best_match_idx >= 0:
                    ...
                    # Per-category stats
                    gt_cat_name = self.category_map.get(
                        gt_category, f"cat_{gt_category}"
                    )
            ...
            for gt in gt_annotations:
                gt_cat_name = self.category_map.get(
                    gt["category_id"], f"cat_{gt['category_id']}"
                )
                per_category_stats[gt_cat_name]["gt_count"] += 1

            # Count predicted categories
            for pred in predictions:
                pred_cat_name = self.category_map.get(
        ...
        classification_accuracy = (
            correct_classifications / correct_detections
    ...
    def print_accuracy_report(self, results: Dict) -> None:
        ...
        print(
            f"Correct Detections (IoU≥{self.iou_threshold}): {metrics['correct_detections']:,}"
        )
        print(f"Correct Classifications: {metrics['correct_classifications']:,}")

        print(f"\n ACCURACY METRICS")
        print(
            f"Detection Recall: {metrics['detection_recall']:.3f} ({metrics['detection_recall']*100:.1f}%)"
        )
        print(
            f"Detection Precision: {metrics['detection_precision']:.3f} ({metrics['detection_precision']*100:.1f}%)"
        )
        print(f"Detection F1-Score: {metrics['detection_f1']:.3f}")
        print(
            f"Classification Accuracy*: {metrics['classification_accuracy']:.3f} ({metrics['classification_accuracy']*100:.1f}%)"
        )
        print(
        ...
        print(
            f"Average Confidence (Correct): {metrics['average_correct_confidence']:.3f}"
        )

        print(f"\n METRIC DEFINITIONS")
        print(
            f"* Classification Accuracy: Correct categories among detected hieroglyphs"
        )
        print(
            f"** Overall Accuracy: Correct detections+classifications among all ground truth"
        )
        ...
        for i, (cat_name, accuracy, count) in enumerate(category_accuracies[:10], 1):
            print(
                f"{i:2d}. {cat_name:8s}: {accuracy:.3f} ({accuracy*100:.1f}%) - {count} samples"
            )
    ...
    def save_detailed_report(
        self, results: Dict, output_file: str = "accuracy_analysis_report.json"
    ) -> None:
    ...
    def create_visualization(
        self, results: Dict, output_file: str = "accuracy_analysis_plots.png"
    ) -> None:
        ...
        for cat_name, stats in per_cat.items():
            if stats["gt_count"] >= 3:  # Minimum samples for meaningful accuracy
                accuracy = (
                    stats["correct_classifications"] / stats["gt_count"]
                    if stats["gt_count"] > 0
                    else 0
                )
        ...
        for img_result in image_results:
            if img_result["gt_count"] > 0:
                ...
                if img_result["correct_detections"] > 0:
                    cls_rate = (
                        img_result["correct_classifications"]
                        / img_result["correct_detections"]
                    )
                else:
                    cls_rate = 0
...
def main():
    ...
    if len(sys.argv) < 3:
        ...
        print(
            "Example: python compute_prediction_accuracy.py hieroglyphs_dataset output/predictions.json 0.5"
        )
        sys.exit(1)
    ...
    try:
        ...
        analyzer = PredictionAccuracyAnalyzer(
            dataset_path, predictions_file, iou_threshold
        )
        ...
        print(
            f"• Overall accuracy: {overall_acc:.1%} (correct detections + classifications)"
        )
        print(
            f"• Classification accuracy: {classification_acc:.1%} (correct categories among detections)"
        )
        print(
            f"• This provides interpretable accuracy metrics complementary to mAP scores"
        )
