...
def compute_accuracy_with_fixed_mappings(iou_threshold=0.5):
    ...
    for pred in predictions:
        pred_cat_name = category_map.get(
            pred["category_id"], f"cat_{pred['category_id']}"
        )
    ...
    detection_precision = (
        correct_detections / total_predictions if total_predictions > 0 else 0
    )
    detection_f1 = (
        2
    ...
    classification_accuracy = (
        correct_classifications / correct_detections if correct_detections > 0 else 0
    )
    overall_accuracy = correct_classifications / total_gt if total_gt > 0 else 0

    avg_confidence = np.mean(confidence_scores) if confidence_scores else 0
    avg_correct_confidence = (
        np.mean(correct_confidence_scores) if correct_confidence_scores else 0
    )
...
def print_results_summary(results):
    ...
    print("=" * 60)
    print(
        f"Dataset: {metrics['total_ground_truth']} ground truth, {metrics['total_predictions']} predictions"
    )
    ...
    print(
        f"Recall:    {metrics['detection_recall']:.1%}  ({metrics['correct_detections']}/{metrics['total_ground_truth']} hieroglyphs found)"
    )
    print(
        f"Precision: {metrics['detection_precision']:.1%}  ({metrics['correct_detections']}/{metrics['total_predictions']} predictions correct)"
    )
    ...
    print(
        f"Accuracy:  {metrics['classification_accuracy']:.1%}  ({metrics['correct_classifications']}/{metrics['correct_detections']} detections classified correctly)"
    )
    print()

    print("OVERALL PERFORMANCE:")
    print(
        f"Accuracy:  {metrics['overall_accuracy']:.1%}  ({metrics['correct_classifications']}/{metrics['total_ground_truth']} hieroglyphs both found AND classified correctly)"
    )
    print()
    ...
    print(
        f"Classification accuracy: 0% → {metrics['classification_accuracy']:.1%} (+{metrics['classification_accuracy']:.1%})"
    )
    print(
        f"Overall accuracy: 0% → {metrics['overall_accuracy']:.1%} (+{metrics['overall_accuracy']:.1%})"
    )
...
def analyze_top_categories(results, category_map, top_n=10):
    ...
    print("=" * 80)
    print(
        f"{'Category':<12} {'GT':<4} {'Pred':<5} {'Det':<4} {'Cls':<4} {'Det%':<6} {'Cls%':<6} {'F1':<6}"
    )
    print("-" * 80)
    ...
    for cat_name, stats in sorted_cats[:top_n]:
        ...
        if gt_count > 0:
            ...
            print(
                f"{cat_name:<12} {gt_count:<4} {pred_count:<5} {det_count:<4} {cls_count:<4} {det_rate:<6.1%} {cls_rate:<6.1%} {f1:<6.3f}"
            )
...
def create_performance_visualization(results, category_map):
    ...
    fig.suptitle(
        "Hieroglyph Detection: Performance Analysis with Fixed Category Mappings",
        fontsize=16,
        fontweight="bold",
    )
    ...
    ax2.bar(
        x - width / 2, gt_counts, width, label="Ground Truth", color="blue", alpha=0.7
    )
    ax2.bar(
        x + width / 2, det_counts, width, label="Detected", color="green", alpha=0.7
    )
    ...
    detection_rates = [
        det_counts[i] / gt_counts[i] if gt_counts[i] > 0 else 0
    ...
    ax4.bar(
        x - width / 2, before_values, width, label="Before Fix", color="red", alpha=0.7
    )
    ax4.bar(
        x + width / 2, after_values, width, label="After Fix", color="green", alpha=0.7
    )
...
def main():
    ...
    print(
        f"Detection performance maintained: {metrics['detection_recall']:.1%} recall, {metrics['detection_precision']:.1%} precision"
    )
    print()
