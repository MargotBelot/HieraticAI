            ...
            if config.category_id_offset != 0:
                ...
                for cat_id, cat_name in sorted(config.categories.items()):
                    ...
                    remapped_categories.append(
                        cat_name
                    )  # Detectron2 uses index-based naming
                ...
                category_names = [
                    ...
                    for cat_id in sorted(config.categories.keys())
                ]
                MetadataCatalog.get(dataset_name).thing_classes = category_names

            logger.info(
                f" Registered {dataset_name}: {len(MetadataCatalog.get(dataset_name).thing_classes)} classes"
            )
...
def setup_config(config: HieroglyphTrainingConfig) -> object:
    ...
    # Backbone freeze: let res3+ fine-tune on papyrus textures
    ...
    # Repeat factor sampling for rare category oversampling
...
def setup_args():
    ...
    parser.add_argument(
        ...
        help="Path to dataset directory (default: ./dataset_clean, generated by Colab notebook)",
    )
    parser.add_argument(
        "--output-dir", default="output_clean_split", help="Output directory for training results"
    )
    parser.add_argument(
        "--model-weights",
        default=None,
    ...
    parser.add_argument(
        "--max-iter", type=int, default=30000, help="Maximum training iterations"
    )
    parser.add_argument(
        "--eval-period", type=int, default=10000, help="Evaluation period (iterations)"
    )
    parser.add_argument(
