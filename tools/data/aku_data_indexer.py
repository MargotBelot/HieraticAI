...
class AKUDataIndexer:
    ...
    def load_all_records(self):
        ...
        for json_file in json_files:
            try:
                ...
                json_relative = (
                    json_file.relative_to(Path.cwd())
                    if json_file.is_absolute()
                    else json_file
                )
                svg_relative = (
                    svg_file.relative_to(Path.cwd())
                    if (svg_file and svg_file.is_absolute())
                    else svg_file
                )
    ...
    def find_svg_file(self, record_id: str, aku_nr: str) -> Optional[Path]:
        ...
        patterns = [f"ht_{record_id}.svg", f"{aku_nr}.svg", f"{record_id}.svg"]
    ...
    def get_gardiner_statistics(self) -> Dict:
        ...
        for code, records in self.gardiner_index.items():
            ...
            readable_count = sum(
                1 for r in records if r["metadata"].get("Lesbarkeit", "") == "ja"
            )
            complete_count = sum(
                1 for r in records if r["metadata"].get("Zustand", "") == "vollständig"
            )
    ...
    def export_index(self, output_path: str):
        ...
        export_data = {
            "gardiner_index": {
                code: records for code, records in self.gardiner_index.items()
            },
    ...
    def create_summary_report(self) -> str:
        ...
        report.append("-" * 50)
        report.append(
            f"{'Code':<8} {'Total':<6} {'SVG':<5} {'Read':<5} {'Comp':<5} {'Description'}"
        )
        report.append("-" * 50)
        ...
        for code, data in sorted_codes[:20]:
            ...
            report.append(
                f"{code:<8} {data['total']:<6} {data['with_svg']:<5} "
