...
class TLADataIndexer:
    ...
    def parse_tla_data(self):
        ...
        for match in matches:
            try:
                ...
                has_hieroglyphs = hieroglyphs and not hieroglyphs.startswith(
    ...
    def get_lemmas_for_hieroglyph(
        self, hieroglyph: str, max_results: int = 10
    ) -> List[Dict]:
        """Get lemma information for a specific hieroglyph."""
        return self.hieroglyph_to_lemmas.get(hieroglyph, [])[:max_results]

    def get_lemmas_for_gardiner_code(
        self, gardiner_code: str, max_results: int = 10
    ) -> List[Dict]:
    ...
    def create_lemma_lookup_index(self) -> Dict:
        ...
        index = {
            ...
            "statistics": {
                ...
                "lemmas_with_hieroglyphs": sum(
                    1 for l in self.lemma_data.values() if l["has_hieroglyphs"]
                ),
                "unique_hieroglyphic_signs": len(self.hieroglyph_to_lemmas),
                "most_frequent_lemmas": sorted(
                    self.lemma_data.values(), key=lambda x: x["frequency"], reverse=True
                )[:20],
            },
        }
    ...
    def create_summary_report(self) -> str:
        ...
        report.append(
            f"Unique hieroglyphic signs: {stats['unique_hieroglyphic_signs']}"
        )
        report.append("")
        ...
        report.append("-" * 60)
        report.append(
            f"{'Rank':<4} {'Freq':<6} {'Transliteration':<15} {'Translation'[:30]}"
        )
        report.append("-" * 60)
        ...
        sign_counts = {
            sign: len(lemmas) for sign, lemmas in self.hieroglyph_to_lemmas.items()
        }
    ...
    if indexer.hieroglyph_to_lemmas:
        ...
        for sign in sample_signs:
            ...
            for lemma in lemmas[:1]:  # Show first match
                print(
                    f"→ {lemma['transliteration']}: {lemma['translation']} (freq: {lemma['frequency']})"
                )
