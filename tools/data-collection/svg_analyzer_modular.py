...
def main():
    ...
    parser.add_argument(
        "--svg-dir", type=str, help="Directory containing SVG files to analyze"
    )
    ...
    try:
        ...
        if args.svg_dir:
            ...
            print(
                f"No SVG directory specified. Use --svg-dir or set svg_directory in config."
            )
            return 1
        ...
        if not svg_files:
            print(
                f"No SVG files found in {svg_directory} matching pattern '{args.pattern}'"
            )
            return 1
            ...
            try:
                ...
                if not args.verbose:
                    progress = progress_bar(
                        i + 1, len(svg_files), prefix="Progress", suffix="Complete"
                    )
                ...
                if svg_content:
                    ...
                    if args.verbose:
                        ...
                        print(
                            f"Dimensions: {metrics['viewbox']['width']:.1f}x{metrics['viewbox']['height']:.1f}"
                        )
                        print(f"Elements: {metrics['complexity']['total_elements']}")
                        print(
                            f"Complexity: {metrics['complexity']['complexity_score']:.2f}"
                        )
                else:
        ...
        if safe_json_save(final_stats, results_file):
            ...
            print(f"\nDimension Statistics:")
            print(
                f"Average size: {dimension_stats['average_width']:.1f} × {dimension_stats['average_height']:.1f}"
            )
            print(
                f"Size range: {dimension_stats['min_width']:.1f}-{dimension_stats['max_width']:.1f} × {dimension_stats['min_height']:.1f}-{dimension_stats['max_height']:.1f}"
            )
            ...
            for elem_type, stats in element_stats.items():
                if stats["total"] > 0:
                    print(
                        f"{elem_type}: {stats['total']} total, avg {stats['average']:.1f} per file"
                    )

            print(f"\nComplexity:")
            print(
                f"Average complexity score: {final_stats['complexity']['average_complexity']:.2f}"
            )
            print(
                f"Complexity range: {final_stats['complexity']['min_complexity']:.2f} - {final_stats['complexity']['max_complexity']:.2f}"
            )
