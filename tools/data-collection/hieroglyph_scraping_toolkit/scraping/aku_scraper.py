...
class AKUPALScraper(BaseScraper):
    ...
    def collect_all_sign_numbers(self) -> List[str]:
        ...
        id_links = self.wait_for_elements(
            (By.CSS_SELECTOR, "a.result-link"), timeout=15
        )
    ...
    def _extract_signs_from_id_page(self, id_url: str, current_id: str) -> List[str]:
        ...
        sign_images = self.wait_for_elements(
            (By.CSS_SELECTOR, "img[alt^='Hieratogramm']"), timeout=10
        )
    ...
    def scrape_sign_metadata(self, sign_id: str) -> Optional[Dict[str, Any]]:
        ...
        metadata_elements = self.wait_for_elements(
            (By.CSS_SELECTOR, ".table tr td"), timeout=15
        )
    ...
    def _structure_metadata(
        self, metadata_list: List[str], sign_id: str
    ) -> Dict[str, Any]:
        """
    ...
    @timing_decorator
    def download_sign_svg(
        self, sign_id: str, output_directory: Optional[str] = None
    ) -> bool:
        """
        ...
        try:
            ...
            self.logger.error(
                f"Unexpected error downloading SVG for sign {sign_id}: {e}"
            )
            return False
    ...
    def scrape_hieratogramm_metadata(self, sign_url: str) -> Dict[str, Any]:
        ...
        sign_data = {"sign_id": sign_id, "url": sign_url}
    ...
    def save_sign_numbers(self, sign_numbers: List[str], output_directory: str) -> bool:
        ...
        try:
            ...
            json_success = safe_json_save(
                sign_numbers, output_dir / "sign_numbers.json"
            )

            # Save TXT file with full URLs
            txt_content = "\n".join(
                f"{self.config.base_url_signs}{number}" for number in sign_numbers
            )
            txt_success = safe_file_write(output_dir / "sign_numbers.txt", txt_content)
    ...
    def save_metadata_batch(
        self, metadata_list: List[Dict[str, Any]], output_directory: str
    ) -> bool:
        """
        ...
        try:
            ...
            json_success = safe_json_save(
                metadata_list, output_dir / "all_metadata.json"
            )
            ...
            txt_success = safe_file_write(output_dir / "all_metadata.txt", txt_content)
    ...
    def get_scraping_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                ...
                / max(1, len(self.collected_signs))
                * 100
            ),
        }
