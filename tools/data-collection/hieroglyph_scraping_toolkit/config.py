...
class WebScrapingConfig:
    ...
    graphemes_url: str = (
        "https://aku-pal.uni-mainz.de/graphemes#dating=Zweite%20Zwischenzeit%2C%2017.%20Dynastie"
    )
...
class PathConfig:
    ...
    output_directory: Path = field(
        default_factory=lambda: Path.home() / "hieroglyph_output"
    )
...
class ConfigManager:
    ...
    def load_defaults(self) -> None:
        ...
        self._config_data = {"paths": {}, "layout": {}, "web_scraping": {}}
    ...
    def get_path_config(self) -> PathConfig:
        ...
        return PathConfig(
            ...
            svg_directory=(
                Path(path_data["svg_directory"])
                if path_data.get("svg_directory")
                else None
            ),
            gardiner_mapping_file=(
                Path(path_data["gardiner_mapping_file"])
                if path_data.get("gardiner_mapping_file")
                else None
            ),
            input_document=(
                Path(path_data["input_document"])
                if path_data.get("input_document")
                else None
            ),
            output_directory=Path(
                path_data.get("output_directory", home_dir / "hieroglyph_output")
            ),
        )
