...
class GardinerExpressionParser:
    ...
    def parse_expression(
        self, expression: str, line_number: int = 0, context: str = "document"
    ) -> ParsedNode:
        """
        ...
        try:
            ...
            self.logger.error(
                f"Failed to parse expression '{expression}' on line {line_number}: {e}"
            )
    ...
    def _parse_tokens(
        self, tokens: List[str], line_number: int, context: str
    ) -> ParsedNode:
        """
        ...
        for i, token in enumerate(tokens):
            try:
                ...
                self.logger.warning(
                    f"Error processing token '{token}' at position {i}: {e}"
                )
        ...
        # Build final tree
        return self._build_tree_from_components(
            node_stack[0], operator_stack[0], context
        )

    def _process_glyph_token(
        self, token: str, line_number: int, context: str
    ) -> ParsedNode:
        """
        ...
        if "&" in token:
            ...
            if parts[0]:
                ...
                self.logger.info(
                    f"Manual placement needed for '{manual_text}' on line {line_number}"
                )
        ...
        if code and self._is_valid_gardiner_code(code):
            ...
            self.logger.warning(
                f"Invalid or empty Gardiner code: '{token}' on line {line_number}"
            )
            return ParsedNode(
                kind="code",
                value=code,
    ...
    def _build_tree_from_components(
        self, nodes: List[ParsedNode], operators: List[str], context: str
    ) -> ParsedNode:
        """
        ...
        for i, operator in enumerate(operators):
            if i + 1 < len(nodes):
                ...
                result = ParsedNode(
                    ...
                    context=context,
                    metadata={"operator": operator, "tree_level": i + 1},
                )

        return result

    def _create_error_recovery_node(
        self, expression: str, error_msg: str
    ) -> ParsedNode:
        """
        ...
        """
        self.logger.error(
            f"Creating error recovery node for: '{expression}' - {error_msg}"
        )
    ...
    def get_parsing_statistics(self) -> Dict[str, Any]:
        ...
        cache_rate = (
            ...
            / max(
                1,
                self.parsing_statistics["cache_hits"]
                + self.parsing_statistics["cache_misses"],
            )
            * 100
        )
    ...
    def get_error_analysis(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "error_rate": len(self.error_patterns)
            / max(1, self.parsing_statistics["total_parsed"]),
        }
