...
class PredictionValidator:
    def __init__(self):
        ...
        if output_dir.exists():
            ...
            training_dirs = [
                d for d in output_dir.iterdir() if d.is_dir() and "training" in d.name
            ]
            if training_dirs:
                ...
                self.predictions_file = (
                    output_dir
                    / "improved_training_20250822_200344"
                    / "coco_instances_results_FIXED.json"
                )
        else:
            self.predictions_file = (
                self.project_root
        ...
        self.aku_data_path = (
            self.project_root / "external_data" / "AKU Westcar Scraping"
        )
        ...
        self.gardiner_codes = {
            ...
            "A14": {
                "name": "man with arms in adoration",
            ...
            "C9": {
                "name": "Hathor",
                "category": "Anthropomorphic deities",
                "unicode": "U+1387D",
            },
            "D200": {
                "name": "eye-paint",
            ...
            "I1": {
                "name": "gecko",
                "category": "Amphibious animals, reptiles",
                "unicode": "U+13188",
            },
            ...
            "O34": {
                "name": "bolt",
                "category": "Buildings and their parts",
                "unicode": "U+13283",
            },
            "P6": {
                "name": "mast",
                "category": "Ships and parts of ships",
                "unicode": "U+132A2",
            },
            "Q19": {
                "name": "hill over hill",
                "category": "Domestic and funerary furniture",
                "unicode": "U+140AB",
            },
            "R10": {
                "name": "festival",
                "category": "Temple furniture and sacred emblems",
                "unicode": "U+132BB",
            },
            "S28": {
                "name": "linen",
                "category": "Crowns, dress, staves",
                "unicode": "U+132F3",
            },
            "S33": {
                "name": "sandal",
            ...
            "U28": {
                "name": "fire-drill",
                "category": "Agriculture, crafts, and professions",
                "unicode": "U+13351",
            },
            "U30": {
                "name": "potter's kiln",
            ...
            "W24": {
                "name": "bowl",
                "category": "Vessels of stone and earthenware",
                "unicode": "U+133CC",
            },
            "W9": {
                "name": "stone bowl",
            ...
            "X8": {
                "name": "bread loaf",
                "category": "Loaves and cakes",
                "unicode": "U+133D9",
            },
            "Y4": {
                "name": "scribe's palette",
                "category": "Writings, games, music",
                "unicode": "U+133DF",
            },
            "Y5": {
                "name": "draught-board",
            ...
            "B1": {
                "name": "woman",
                "category": "Woman and her occupations",
                "unicode": "U+13050",
            },
            "D1": {
            ...
            "M15": {
                "name": "clump of papyrus",
                "category": "Trees and plants",
                "unicode": "U+131C7",
            },
            "M18": {
                "name": "clump of papyrus with flowers",
                "category": "Trees and plants",
                "unicode": "U+131CD",
            },
            "M23": {
    ...
    def _validate_paths(self):
        ...
        if not self.aku_data_path.exists():
            st.warning(f"[WARNING] AKU database not found at: {self.aku_data_path}")
            st.info(
                "💡 AKU database features will be disabled. To enable, ensure the 'external_data/AKU Westcar Scraping' directory exists."
            )

        if not self.aku_index_path.exists():
            st.warning(f"[WARNING] AKU index not found at: {self.aku_index_path}")
            st.info(
                "💡 Run the AKU indexer to generate the index: `python tools/data/aku_data_indexer.py`"
            )

        if not self.tla_index_path.exists():
            st.warning(f"[WARNING] TLA index not found at: {self.tla_index_path}")
            st.info(
                "💡 Run the TLA indexer to generate the index: `python tools/data/tla_data_indexer.py`"
            )

        # Fail only for critical missing paths
        if missing_paths:
            st.error("[FAIL] Critical files missing:")
            for path in missing_paths:
                st.error(f"  • {path}")
            st.error(
                "Please ensure you've completed training and have the correct directory structure."
            )
            st.stop()
    ...
    def load_aku_index(_self):
        ...
        try:
            if _self.aku_index_path.exists():
                ...
                st.warning(
                    f"AKU index not found at {_self.aku_index_path}. Consider running the indexer."
                )
                return {}
    ...
    def load_tla_index(_self):
        ...
        try:
            if _self.tla_index_path.exists():
                ...
                st.warning(
                    f"TLA index not found at {_self.tla_index_path}. Run the TLA indexer to generate it."
                )
                return {}
        except Exception as e:
    ...
    def get_similar_aku_signs(
        self, gardiner_code: str, max_results: int = 3
    ) -> List[Dict]:
    ...
    def get_tla_lemmas_for_sign(self, gardiner_code: str) -> List[Dict]:
        ...
        if hieroglyph and self.tla_index:
            hieroglyph_index = self.tla_index.get(
                "by_hieroglyph", self.tla_index.get("hieroglyph_index", {})
            )
            ...
            if fallback_hieroglyph and self.tla_index:
                ...
                if lemmas:
                    ...
                    for lemma in lemmas:
                        ...
                        enhanced_lemma["fallback_note"] = (
                            f"Data from similar sign {fallback_code}"
                        )
        ...
        manual_entries = {
            "A14": [
                {
                    "hieroglyphs": "",
                    "transliteration": "dw",
                    "translation": "To worship; to adore",
                    "frequency": 0,
                    "lemma_id": "MANUAL_A14",
                    "source": "Gardiner + Manual",
                }
            ],
            "A49": [
                {
                    "hieroglyphs": "",
                    "transliteration": "mw",
                    "translation": "Asiatic; Syrian (determinative)",
                    "frequency": 0,
                    "lemma_id": "MANUAL_A49",
                    "source": "Gardiner + Manual",
                }
            ...
            "C9": [
                {
                    ...
                    "translation": "Hathor (goddess determinative)",
                    "frequency": 0,
                    "lemma_id": "MANUAL_C9",
                    "source": "Gardiner + Manual",
                }
            ...
            "F9": [
                {
                    "hieroglyphs": "",
                    "transliteration": "pẖr",
                    "translation": "Leopard (determinative)",
                    "frequency": 0,
                    "lemma_id": "MANUAL_F9",
                    "source": "Gardiner + Manual",
                }
            ...
            "I1": [
                {
                    "hieroglyphs": "",
                    "transliteration": "sf",
                    "translation": "Gecko; lizard (determinative)",
                    "frequency": 0,
                    "lemma_id": "MANUAL_I1",
                    "source": "Gardiner + Manual",
                }
            ...
            "M5": [
                {
                    "hieroglyphs": "",
                    "transliteration": "jšd",
                    "translation": "Tree; wood (determinative)",
                    "frequency": 0,
                    "lemma_id": "MANUAL_M5",
                    "source": "Gardiner + Manual",
                }
            ...
            "N90": [
                {
                    "hieroglyphs": "",
                    "transliteration": "mw",
                    "translation": "Water; liquid (determinative)",
                    "frequency": 0,
                    "lemma_id": "MANUAL_N90",
                    "source": "Gardiner + Manual",
                }
            ...
            "Q19": [
                {
                    "hieroglyphs": "",
                    "transliteration": "qrrt",
                    "translation": "Hill; mound (determinative)",
                    "frequency": 0,
                    "lemma_id": "MANUAL_Q19",
                    "source": "Gardiner + Manual",
                }
            ...
            "S33": [
                {
                    "hieroglyphs": "",
                    "transliteration": "tbw.t",
                    "translation": "Sandal; shoe",
                    "frequency": 0,
                    "lemma_id": "MANUAL_S33",
                    "source": "Gardiner + Manual",
                }
            ...
            "Y4": [
                {
                    "hieroglyphs": "",
                    "transliteration": "mn",
                    "translation": "Gaming piece; draughtsman",
                    "frequency": 0,
                    "lemma_id": "MANUAL_Y4",
                    "source": "Gardiner + Manual",
                }
            ...
            "Z3A": [
                {
                    "hieroglyphs": "",
                    "transliteration": "3",
                    "translation": "Three (strokes); plural marker",
                    "frequency": 0,
                    "lemma_id": "MANUAL_Z3A",
                    "source": "Gardiner + Manual",
                }
            ...
            "D45": [
                {
                    "hieroglyphs": "",
                    "transliteration": "ḏsr",
                    "translation": "Arm with sacred objects",
                    "frequency": 0,
                    "lemma_id": "MANUAL_D45",
                    "source": "Gardiner + Manual",
                }
            ...
            "M22A": [
                {
                    "hieroglyphs": "",
                    "transliteration": "nn",
                    "translation": "Rush; reed plant (determinative)",
                    "frequency": 0,
                    "lemma_id": "MANUAL_M22A",
                    "source": "Gardiner + Manual",
                }
            ],
        }
        ...
        try:
            ...
            if not Path(svg_path).exists():
                ...
                if local_svg_path.exists():
                    ...
                    return (
                        f'<div style="color: red;">SVG file not found: {filename}</div>'
                    )
    ...
    def display_aku_reference_signs(self, gardiner_code: str, container):
        ...
        with container:
            ...
            if similar_signs:
                ...
                st.markdown(
                    f"*Found {len(similar_signs)} similar signs in AKU Westcar database*"
                )
                    ...
                    with st.container():
                        ...
                        with col1:
                            ...
                            if sign.get("svg_path"):
                                svg_html = self.get_svg_as_html(
                                    sign["svg_path"], width=60
                                )
                ...
                if stats:
                    st.markdown(
                        f"**Database Stats:** {stats.get('total', 0)} total, "
                ...
                st.markdown(f"### AKU Reference Signs for `{gardiner_code}`")
                st.info(
                    f"No similar signs found in AKU database for `{gardiner_code}`."
                )

                # Show some available codes for reference
                if self.aku_index:
                    available_codes = list(
                        self.aku_index.get("gardiner_index", {}).keys()
                    )[:10]
                    if available_codes:
                        st.write(
                            "Available codes (sample): " + ", ".join(available_codes)
                        )
    ...
    def display_tla_lemma_info(self, gardiner_code: str, container):
        ...
        with container:
            ...
            if lemmas:
                ...
                st.markdown(
                    f"*Found {len(lemmas)} related lemmas in TLA database (showing top 5)*"
                )
                ...
                if hieroglyph and self.tla_index:
                    ...
                    st.markdown(
                        f"**TLA Database Stats:** {len(lemmas)} lemmas for this sign, {total_signs} total signs indexed"
                    )
                ...
                if hieroglyph:
                    st.info(
                        f"No lemma information found for hieroglyph '{hieroglyph}' ({gardiner_code}) in TLA database."
                    )
                else:
                    st.info(
                        f"Could not convert Gardiner code '{gardiner_code}' to Unicode hieroglyph for TLA lookup."
                    )
    ...
    def visualize_predictions_enhanced(
        self,
        image_path,
        predictions,
        image_id,
...
def main():
    ...
    st.markdown(
        "**Interactive validation interface for hieratic character predictions on the Westcar Papyrus**"
    )
    ...
    if validator.aku_index:
        ...
        st.sidebar.success(
            f"Connected: {total_aku_records} records, {total_gardiner_codes} codes"
        )
    else:
    ...
    if validator.tla_index:
        ...
        total_tla_lemmas = (
            len(by_lemma)
            if by_lemma
            else sum(len(lemmas) for lemmas in by_hieroglyph.values())
        )
        st.sidebar.success(
            f"Connected: {total_tla_lemmas} lemmas, {total_tla_signs} signs"
        )
    else:
    ...
    available_images = list(
        set([pred.get("image_id", "unknown") for pred in predictions_data])
    )
    ...
    min_confidence = st.sidebar.slider(
        "Minimum Confidence", min_value=0.0, max_value=1.0, value=0.3, step=0.05
    )
    ...
    **Recommendation:** Start validation with high confidence predictions to establish baseline accuracy, then work through medium and low confidence detections.
    ...
    for i, pred in enumerate(predictions_data):
        if (
            pred.get("image_id") == selected_image
            and pred.get("score", 0) >= min_confidence
        ):
    ...
    if filter_mode != "all":
        ...
        st.info(
            f"**Filter Active:** Showing {filter_labels.get(filter_mode, filter_mode)} - Found {len(image_predictions)} predictions"
        )

    if not image_predictions:
        st.warning(
            f"No predictions found for image {selected_image} with confidence >= {min_confidence}"
        )
        return
    ...
    for split in ["test", "val", "train"]:
        try:
            ...
            split_image_map = {
                img_info["id"]: img_info["file_name"]
    ...
    total_predictions = len(
        [p for p in predictions_data if p.get("score", 0) >= min_confidence]
    )
    ...
    if validated_count > 0:
        ...
        with col3:
            progress = (
                (validated_count / total_predictions) * 100
                        ...
                        confidence_data.append(
                            {
                                ...
                                "gardiner_code": validator.get_gardiner_info(
        ...
        st.info(
            "No predictions have been validated yet. Start validating to see statistics!"
        )
            ...
            if validated_predictions:
                ...
                for pred in predictions_data:
                    if pred.get("score", 0) >= min_confidence:
                        pred_key = f"{pred.get('image_id', 'unknown')}_{predictions_data.index(pred)}"
                        if pred_key in validated_predictions:
                            gardiner_code, gardiner_info = validator.get_gardiner_info(
                                pred.get("category_id", 0)
                            )
                            export_data.append(
                                {
                                    ...
                                    "gardiner_name": gardiner_info["name"],
                                    "confidence": pred.get("score", 0),
                                    "validation_status": validated_predictions[
                                        pred_key
                                    ],
                                    "bbox": pred.get("bbox", []),
                                    "timestamp": datetime.now().isoformat(),
